...
fer_dataset_test = Four4All(csv_file='FER2013/subConjuntoFER2013_paraExperimento/test_explicabilidad_Remapped_100imagenesOriginales.csv',
                            img_dir='FER2013/subConjuntoFER2013_paraExperimento', transform=transform)

''' fer_dataset_test = Four4All(csv_file='FER2013/test_labels_remapped.csv',
                            img_dir='FER2013/test', transform=transform) '''

data_test_loader = DataLoader(fer_dataset_test, batch_size=16, shuffle=False)
test_image, test_label = next(iter(data_test_loader))
criterion = torch.nn.CrossEntropyLoss()                 # Criterio: para clasificacion multiclase, compara outputs (logits) con labels (indice entero)
...
checkpoint = torch.load('./Weights/fer_model.pth')             # Pesos entregados previamente por autor 
...
disp.plot(cmap=plt.cm.YlGn, ax=ax, colorbar=True, values_format='.2f')
plt.xticks(rotation=45, fontsize=9)
plt.yticks(fontsize=9)
plt.title(f"ResEmoteNet con FER2013. Acc={final_test_acc*100:.2f}%", fontsize=11, fontweight='semibold')
...
df_metrics.set_index('Clase').plot(kind='bar', figsize=(6, 4), ylim=(0, 1), colormap='viridis')
